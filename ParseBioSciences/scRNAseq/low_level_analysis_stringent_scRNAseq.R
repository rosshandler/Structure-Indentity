...
setwd('/data1/ivanir/Ilaria2023/ParseBS/newvolume/analysis/sCell/QC_stringent')

pdf("cell_complexity.pdf")
qplot(lib.sizes, ngenes, col = ifelse(ngenes < 600, "drop", "keep")) +
  scale_x_log10() +
  scale_y_log10() +
  theme_minimal() + 
...
dim(counts[,ngenes > 600 & lib.sizes > 1000])
#[1] 62704 20269

counts   <- counts[,ngenes > 600 & lib.sizes > 1000]
metadata <- metadata[ngenes > 600 & lib.sizes > 1000,]
lib.sizes <- colSums(counts)
ngenes    <- colSums(counts > 0)
...
saveRDS(sce,paste0(path2data,"sce_stringent.rds"))
