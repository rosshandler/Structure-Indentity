...
pdf("milo_noMG_adapted.pdf",width=12,height=6)
plotNhoodGraphDA_adapted(milo.obj, milo.res2, alpha=0.5) +
  plot_layout(guides="collect")
dev.off()

pdf("milo_Diss.pdf",width=12,height=6)
plotNhoodGraphDA(milo.obj, milo.res3, alpha=0.5) +
  plot_layout(guides="collect")
dev.off()

pdf("milo_Diss_adapted.pdf",width=12,height=6)
plotNhoodGraphDA_adapted(milo.obj, milo.res3, alpha=0.5) +
  plot_layout(guides="collect")
dev.off()

pdf("milo_Fast.pdf",width=12,height=6)
plotNhoodGraphDA(milo.obj, milo.res1, alpha=0.5) +
  plot_layout(guides="collect")
dev.off()

saveRDS(milo.obj, "miloR_DA.rds")

milo.design <- as.data.frame(xtabs(~ morphology_clus + sample, data=meta))
milo.design <- milo.design[milo.design$Freq > 0, ]
rownames(milo.design) <- milo.design$sample

milo.res1 <- testNhoods(milo.obj, design=~morphology_clus+0, design.df=milo.design,
